...
class admin_cycleimage extends ecjia_admin {
	...
	public function __construct() {
		...
		ecjia_screen::$current_screen->add_nav_here(new admin_nav_here(__('iPad轮播图'), RC_Uri::url('mobile/admin_cycleimage/init')));
	}
	...
	public function init() {
		...
		ecjia_screen::$current_screen->add_nav_here(new admin_nav_here(__('iPad轮播图')));
		...
		$this->assign('action_link_special', array('text' => __('添加轮播图'), 'href' => RC_Uri::url('mobile/admin_cycleimage/add')));
	...
	public function add() {
		...
		if (empty($_POST['step'])) {
			...
			$this->assign('action_link', array('text' => __('轮播图列表'), 'href' => RC_Uri::url('mobile/admin_cycleimage/init')));
...
// 					$src = $info['savepath'] . '/' . $info['savename'];
			...
			$links[] = array('text' => __('轮播图列表'), 'href' => RC_Uri::url('mobile/admin_cycleimage/init'));

			ecjia_admin::admin_log($_POST['img_text'], 'add', 'mobile_cycleimage');
			$this->showmessage('添加轮播图成功！', ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('links' => $links , 'pjaxurl' => RC_Uri::url('mobile/admin_cycleimage/add')));
		}
	}
...
// 				@unlink(RC_Upload::upload_path() . $rt['src']);
		    ...
		    $this->showmessage('编辑轮播图成功！', ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('links' => $links , 'pjaxurl' => RC_Uri::url('mobile/admin_cycleimage/init')));
		}
	}
...
// 			@unlink(RC_Upload::upload_path() . $rt['src']);
	...
	public function edit_sort() {
		...
		if (!is_numeric($order)) {
			...
			$this->showmessage('轮播图列表排序操作成功！', ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_uri::url('mobile/admin_cycleimage/init')) );
		}
	}
